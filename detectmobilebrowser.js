...
function hasTouch() {
    const matchMedia = (window.matchMedia || (() => ({ matches: false }))).bind(window);
    const supportsTouchEvents = 'ontouchstart' in window || ('TouchEvent' in window && matchMedia('(any-pointer: coarse)').matches);
    return (window.navigator.maxTouchPoints || 0) > 0 || supportsTouchEvents;
}
